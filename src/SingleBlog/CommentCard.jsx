...
import { isCommentAuthor } from "../utility";
...
export default function CommentCard({ comment,blogId, setSingleBlog}) {
  ...
  const handelCilick = async()=>{
    ...
    try {
        ...
        const response = await api.delete(`http://localhost:3000/blogs/${blogId}/comment/${comment?.id}`);
    
        // Check if the request was successful
        if (response.status === 200) {
          console.log('Data deleted successfully',response);
          setSingleBlog((prev) => ({ ...prev, comments: response?.data?.comments }));
  ...
  return (
    <div key={comment.id} className="relative flex items-start space-x-4 my-8">
      ...
      {/* for delete */}
      {isCommentAuthor(comment?.author?.id, userId) && (
        <div className="absolute right-0 top-0">
          ...
          <div className="action-modal-container">
            {showDots && (
              <>
                <button onClick={handelCilick} className="action-menu-item hover:text-red-500">
